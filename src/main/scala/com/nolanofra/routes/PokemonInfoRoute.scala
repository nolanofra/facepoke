...
class PokemonInfoRoute private (private val pokemonService: PokemonService) {

  def infoPokemon: HttpRoutes[IO] = {
    val dsl = Http4sDsl[IO]
    import dsl._

    HttpRoutes
      .of[IO] { case GET -> Root / "pokemon" / name =>
        pokemonService
...
object PokemonInfoRoute {
  def apply(pokemonService: PokemonService) = new PokemonInfoRoute(pokemonService)
}
